...
for(f in files) {
  suppressWarnings(tbl <- readr::read_csv(f, guess_max=2))
  rows <- dim(tbl)[1]
...
t.test(expsum1$Mean_Ratio[expsum1$Treatment != 'DMSO' & expsum1$Line == 'Reporter'], expsum1$Mean_Ratio[expsum1$Treatment == 'DMSO' & expsum1$Line == 'Reporter'], paired=FALSE, var.equal=FALSE)

write.table(expdata, file.path(dir, 'summary-full.txt'), row.names=FALSE) 
